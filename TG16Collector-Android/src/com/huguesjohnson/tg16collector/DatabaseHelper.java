...
public class DatabaseHelper extends SQLiteOpenHelper{
	...
	private static final String DB_NAME="tg16collector";
	private static final String TABLE_NAME="mytg16collection";
	private static final String SELECT_FROM="SELECT recordId,title,havegame,havebox,havecase,haveinstructions,wishlist FROM ";
	...
	private void loadGameList(Context context) throws IOException,NotFoundException,SQLException{
		...
		while(indexOf>-1){
			...
			if((split.length>1)&&(split[1].length()>0)){
				currentKeyword=split[1];
			}else{
				currentKeyword=currentGame.replace(" ","+");
			}
			try{
				database.execSQL(buildInsert(recordId,currentGame,currentKeyword));
	...
	private String buildInsert(int recordId,String gameTitle,String keyword){
		StringBuffer insertInto=new StringBuffer();
		insertInto.append("INSERT INTO ");
		insertInto.append(TABLE_NAME);
		insertInto.append(" ('recordId','title','keyword','havegame','havecase','havebox','haveinstructions','wishlist') VALUES (");
	...
	public ArrayList<TG16Record> getAllGames(){
		return(getAllGames(null));
	}
	
	public ArrayList<TG16Record> getAllGames(String titleFilter){
	...
	public ArrayList<TG16Record> getMyGames(){
		return(getMyGames(null));
	}
	
	public ArrayList<TG16Record> getMyGames(String titleFilter){
		StringBuffer query=new StringBuffer(SELECT_FROM);
		query.append(TABLE_NAME);
	...
	public ArrayList<TG16Record> getWishList(){
		return(getWishList(null));
	}

	public ArrayList<TG16Record> getWishList(String titleFilter){
	...
	public ArrayList<TG16Record> getMissingGames(boolean missingBoxes,boolean missingInstructions,boolean missingCase){
		return(getMissingGames(missingBoxes,missingInstructions,missingCase,null));
	}
	
	public ArrayList<TG16Record> getMissingGames(boolean missingBoxes,boolean missingInstructions,boolean missingCase,String titleFilter){
	...
	public TG16Record getGame(int recordId){
		String sql=SELECT_FROM+TABLE_NAME+" WHERE recordId="+recordId;
		ArrayList<TG16Record> list=queryDatabase(sql);
		return(list.get(0));
	}
	...
	public void updateGame(TG16Record record) throws SQLException{
	...
	private ArrayList<TG16Record> queryDatabase(String sql) throws IllegalArgumentException{
		ArrayList<TG16Record> list=new ArrayList<TG16Record>();
		...
		if(cursor!=null){
			...
			while(cursor.moveToNext()){
				...
				TG16Record record=new TG16Record();
